    ...
    <div class="grid grid-cols-1 gap-8 md:grid-cols-1 lg:grid-cols-1">
        @foreach($posts as $post)
            <article class="bg-gray-50 rounded-lg p-6 hover:shadow-sm transition-shadow duration-200">
                ...
                <time datetime="2024-12-05" class="text-gray-400">{{ \Carbon\Carbon::parse($post['created_at'])->format('j M Y') }}</time>
                <span class="px-2 py-1 bg-purple-50 text-purple-600 rounded text-xs">{{ $post['category'] }}</span>
                </div>
                <div class="mb-6">
                ...
                <img src="{{ $post['author_avatar'] }}" alt="" class="w-8 h-8 rounded-full" />
                <div class="text-xs">
